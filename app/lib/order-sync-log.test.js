...
import { SYNC_WINDOWS, windowLabel } from "./sync-windows.js";
...
describe("SYNC_WINDOWS", () => {
  ...
  it("carries the empty value, which means the scheduled run itself", () => {
    const current = SYNC_WINDOWS.find((w) => w.value === "");
    assert.ok(current, "the current/no-window option must exist");
    assert.equal(current.hours, 0);
  });

  it("states every other value in hours, matching its own value", () => {
    for (const w of SYNC_WINDOWS.filter((x) => x.value !== "")) {
  ...
  it("covers the ranges the page offers", () => {
    assert.deepEqual(SYNC_WINDOWS.map((w) => w.hours), [0, 1, 2, 6, 12, 24, 48]);
  });
});
...
describe("windowLabel", () => {
  ...
  it("still says something usable for a range that is not on the list", () => {
    ...
    // 0 is the scheduled run, which has no window to name.
